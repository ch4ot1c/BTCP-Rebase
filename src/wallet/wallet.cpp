...
{
    {
        ...
        for (std::pair<const uint256, CWalletTx>& wtxItem : mapWallet) {
            for (mapNoteData_t::value_type& item : wtxItem.second.mapNoteData) {
                ...
                if (nd->witnessHeight < pindex->nHeight) {
                    ...
                    if (nd->witnesses.size() > NOTE_WITNESS_CACHE_SIZE) {
                        nd->witnesses.pop_back();
                    }
                }
            }
        }
        if (nWitnessCacheSize < NOTE_WITNESS_CACHE_SIZE) {
            nWitnessCacheSize += 1;
        }
        ...
        for (const CTransactionRef& txRef : pblock->vtx) {
            ...
            for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
                ...
                for (uint8_t j = 0; j < jsdesc.commitments.size(); j++) {
                    ...
                    if (txIsOurs) {
                        ...
                        if (mapWallet.find(hash)->second.mapNoteData.count(jsoutpt) &&
                                mapWallet.find(hash)->second.mapNoteData[jsoutpt].witnessHeight < pindex->nHeight) {
                            CNoteData* nd = &(mapWallet.find(hash)->second.mapNoteData[jsoutpt]);
                            if (nd->witnesses.size() > 0) {
